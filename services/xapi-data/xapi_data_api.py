...
def sql(q):
    return sh("sudo docker exec -i example-postgres psql -U sub2api -d sub2api -AtF '|' -v ON_ERROR_STOP=1", q)
...
def clear_cache():
    try:
        sh('sudo docker exec example-redis redis-cli FLUSHDB >/dev/null 2>&1 || true')
    except Exception:
        pass

def restart_gateway():
    try:
        sh('sudo docker restart nw-api >/dev/null && sleep 3')
    except Exception:
        pass
