#!/usr/bin/env python3
import json, secrets, subprocess, urllib.parse, re, bcrypt
...
def sql(q):
    return sh("sudo docker exec -i postgres-container psql -U app_user -d app_db -AtF '|' -v ON_ERROR_STOP=1", q)
...
def clear_cache():
    try:
        sh('sudo docker exec redis-container redis-cli FLUSHDB >/dev/null 2>&1 || true')
    except Exception:
        pass

def restart_gateway():
    try:
        sh('sudo docker restart your-gateway >/dev/null && sleep 3')
    except Exception:
        pass
