...
KEYS={1:[[1,'NW-API Preview Admin Key','active','sk-example-admin',0,0,'never']],2:[[2,'NW-API Preview Test Key','active','sk-example-test',0,0,'never']]}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path.endswith('/keys'):
            uid=int(data.get('uid') or 1); kid=len(KEYS.get(uid,[]))+1; key='sk-example-'+secrets.token_hex(8); KEYS.setdefault(uid,[]).append([kid,data.get('name') or 'Preview Key','active',key,0,0,'never']); return send_json(self, {'ok':True,'key':key})
        if path.endswith('/keys/rotate'):
            return send_json(self, {'ok':True,'key':'sk-example-'+secrets.token_hex(8)})
        return send_json(self, {'ok':True,'mock':True})
