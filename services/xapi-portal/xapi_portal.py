...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL='https://api.example.com/v1'; APP_VERSION='v.0.011'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
...
def update_state(force=False):
    ...
    try:
        ...
        fetch=run_cmd(['git','fetch','--quiet','origin',branch,'--tags'], timeout=20)
        ...
        data={'ok':True,'available':bool(local and remote and local!=remote),'local':local,'remote':remote,'remote_tag':remote_tag,'branch':branch,'error':''}
...
def inline_update_log(st):
    try:
        local=st.get('local') or 'HEAD'
        remote=st.get('remote') or 'origin/main'
        out=run_cmd(['git','log','--oneline','--no-decorate',f'{local}..{remote}','--'], cwd=REPO_DIR, timeout=8).stdout
        items=[]
        for raw in [x for x in out.splitlines()[:5] if x.strip()]:
            parts=raw.split(' ',1)
            sha=parts[0]
            msg=parts[1] if len(parts)>1 else ''
            if re.fullmatch(r'[0-9a-fA-F]{7,40}', sha):
                items.append(f'<li><a href="https://github.com/ahnerjack/nw-api-stack/commit/{esc(sha)}" target="_blank" rel="noopener">{esc(sha)}</a> {esc(msg)}</li>')
            else:
                items.append(f'<li>{esc(raw)}</li>')
        return '<div class="release-log"><b>更新日志</b><ul>'+''.join(items)+'</ul></div>' if items else '<div class="release-log muted">发现新版本，暂无提交摘要。</div>'
    except Exception as e:
        return f'<div class="release-log muted">更新日志读取失败：{esc(e)}</div>'
...
def release_version_menu(label, user=None, can_update=False, st=None):
    ...
    if user and user.get('role')=='admin' and can_update:
        update_html=f'<form method="post" action="admin-update/apply" class="release-update-form" onsubmit="return nwInlineUpdate(this)">{csrf_field(user)}<button type="submit">立即更新</button><div class="inline-update-result"></div></form>'
...
def version_update_widget(user):
    ...
    if st.get('available'):
        ...
        version=release_version_menu(label, user, True, st)
        return f'<span class="version-row">{version}<span class="update-badge" title="发现新版本">NEW</span></span>'
    return release_version_menu(label, user, False, st)
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path.endswith('/admin-update/apply'):
            if u.get('role')!='admin':
            ...
            cmd='cd '+shlex.quote(REPO_DIR)+' && git fetch --quiet origin '+shlex.quote(branch)+' --tags && git reset --hard '+shlex.quote('origin/'+branch)+' && ./scripts/deploy-aliyun.sh'
            log_cmd='{ echo ==== $(date) admin='+shlex.quote(u['email'])+' local='+shlex.quote(st.get('local',''))+' remote='+shlex.quote(st.get('remote',''))+' branch='+shlex.quote(branch)+'; '+cmd+'; } >> '+shlex.quote(UPDATE_LOG)+' 2>&1'
            subprocess.Popen(['bash','-lc', log_cmd], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
