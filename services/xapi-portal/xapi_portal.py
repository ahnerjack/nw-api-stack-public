...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL='https://api.example.com/v1'; APP_VERSION='v.0.008'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
...
def release_version_menu(label, user=None, can_update=False, st=None):
    release_url='https://github.com/ahnerjack/nw-api-stack-public/releases/tag/v.0.008'
    link_text='更新日志' if can_update else '查看发布'
    update_html=''
    if user and user.get('role')=='admin' and can_update:
        update_html=f'<form method="post" action="/admin-update/apply" class="release-update-form" onsubmit="return nwInlineUpdate(this.querySelector(\'button\'))">{csrf_field(user)}<button type="submit">立即更新</button><div class="inline-update-result"></div></form>'
