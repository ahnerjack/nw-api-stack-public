...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL='https://api.example.com/v1'; APP_VERSION='V0.0.004'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
...
def version_update_widget(user):
    ...
    if not user or user.get('role')!='admin':
        return f'<small class=version-pill>{label}</small>'
    st=update_state(False)
    if st.get('available'):
        ensure_admin_update_notice(st)
        # 侧栏只显示版本与 NEW 徽标，避免 details 弹层挤坏导航；详情统一进 /admin-update。
        return f'<span class=version-row><small class=version-pill>{label}</small><a class=update-badge href=/admin-update title=发现新版本>NEW</a></span>'
    return f'<small class=version-pill>{label}</small>'
...
def public_page(title, body):
    ...
    return f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{css}</head><body><main class="main" style="margin:auto"><div class="top"><a class="logo" href="/home" style="color:#101828"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span class="brand-copy"><span class="brand-name">{BRAND}</span><small class="version-pill">{APP_VERSION}</small><small class="muted">AI API Gateway</small></span></a><div class="actions">{nav}</div></div><h1>{esc(title)}</h1>{body}<footer class="muted" style="margin-top:34px">© 2026 NW-API · <a href="/privacy">隐私政策</a> · <a href="/terms">服务条款</a> · <a href="/docs">接入文档</a></footer></main></body></html>'
