...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL='https://api.example.com/v1'; APP_VERSION='v0.0.003'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
...
def mask_key(k):
    ...
    if len(s) <= 10: return (s[:1] + '****' + s[-1:]) if len(s) > 1 else '****'
    return s[:6] + ('*' * max(len(s)-10, 4)) + s[-4:]
...
def version_update_widget(user):
    ...
    if not user or user.get('role')!='admin':
        return f'<small style="color:#94a3b8">{label}</small>'
    st=update_state(False)
    if st.get('available'):
        ensure_admin_update_notice(st)
        tip=f'发现新版本：当前 {esc(st.get("local"))}，Git 最新 {esc(st.get("remote"))}'
        return (f'<details class="ver-update has-update"><summary title="{tip}">{label}<span>NEW</span></summary>'
                f'<div><b>发现新版本</b><p>当前：{esc(st.get("local"))}<br>最新：{esc(st.get("remote"))}</p>'
                f'<p class="muted">只更新功能代码，不覆盖用户、数据库、Sub2API 连接和密钥配置。</p>'
                f'<form method="post" action="/admin-update/apply">{sensitive_fields(user,"更新备注")}<button class="btn">立即更新</button></form>'
                f'<a class="btn btn2" href="/admin-update">查看更新内容</a></div></details>')
    return f'<small style="color:#94a3b8">{label}</small>'
...
def shell(title, body, user, active='dashboard'):
    ...
    return f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="layout"><aside class="side"><a class="logo" href="/dashboard"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>{BRAND}<br>{version_update_widget(user)}</span></a><div class="nav">{nav}</div></aside><main class="main"><div class="top">{heading}<a class="btn btn2" href="/">返回首页</a></div>{admin_update_banner(user)}{body}</main></div></body></html>'
...
def public_page(title, body):
    ...
    return f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{css}</head><body><main class="main" style="margin:auto"><div class="top"><a class="logo" href="/home" style="color:#101828"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>{BRAND}<br><small class="muted">AI API Gateway · {APP_VERSION}</small></span></a><div class="actions">{nav}</div></div><h1>{esc(title)}</h1>{body}<footer class="muted" style="margin-top:34px">© 2026 NW-API · <a href="/privacy">隐私政策</a> · <a href="/terms">服务条款</a> · <a href="/docs">接入文档</a></footer></main></body></html>'
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ['/dashboard','/onboarding','/profile','/notifications','/keys','/keys/view','/usage','/billing','/docs','/users-admin','/admin-console','/admin-usage','/user-detail','/api-logs','/rollback','/mail-logs','/announcements','/healthz','/changelog','/export.csv'] and not u: return self.redirect('/login')
        try:
    ...
    def do_POST(self):
        ...
        sensitive_paths={'/users-admin/update','/users-admin/delete','/users-admin/plan','/users-admin/models','/balance/change','/notifications/send','/admin-update/apply'}
        ...
        if path=='/admin-update/apply' and u['role']=='admin':
        ...
        if path=='/keys/rotate':
            ...
            return self.sendh(shell('API Key 已更新',f'<div class="card"><p>请立即复制保存，新 Key 只在这里显示一次：</p><pre>{esc(key)}</pre><p class="muted">列表页会继续显示为 {esc(mask_key(key))}</p><a class="btn" href="/keys">返回 API Key</a></div>',u,'keys'))
        ...
        if path=='/keys/create':
            ...
            key=res.get('key','')
            return self.sendh(shell('新 API Key',f'<div class="card"><p>请立即复制保存，只显示这一次：</p><pre>{esc(key)}</pre><a class="btn" href="/keys">返回</a></div>',u,'keys'))
        ...
        self.redirect('/dashboard')
    def login(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>登录控制台</h1><p class="err">{esc(err)}</p><input name="email" placeholder="邮箱" autofocus><input name="password" type="password" placeholder="密码"><button class="btn">登录</button><p class="muted">没有账号？<a href="/register">注册</a> · <a href="/forgot">忘记密码</a></p></form></div></body></html>')
    def register(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>注册账号</h1><p class="err">{esc(err)}</p><input name="email" placeholder="邮箱"><button class="btn btn2" formaction="/send-code" formmethod="post">发送邮箱验证码</button><input name="code" placeholder="邮箱验证码"><input name="password" type="password" placeholder="登录密码，至少 8 位"><input name="username" placeholder="昵称，可选"><button class="btn">注册普通会员账号</button><p class="muted">已有账号？<a href="/login">登录</a></p></form></div></body></html>')
    def forgot(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark"><svg class="nw-logo" viewBox="0 0 72 72" aria-hidden="true" shape-rendering="geometricPrecision"><defs><linearGradient id="portalBg" x1="8" y1="7" x2="64" y2="65" gradientUnits="userSpaceOnUse"><stop stop-color="#0F172A"/><stop offset="1" stop-color="#020617"/></linearGradient><linearGradient id="portalRing" x1="17" y1="15" x2="55" y2="57" gradientUnits="userSpaceOnUse"><stop stop-color="#67E8F9"/><stop offset="0.48" stop-color="#F8FAFC"/><stop offset="1" stop-color="#A78BFA"/></linearGradient></defs><rect x="6" y="6" width="60" height="60" rx="20" fill="url(#portalBg)"/><rect x="9" y="9" width="54" height="54" rx="18" fill="rgba(255,255,255,.035)" stroke="rgba(255,255,255,.12)" stroke-width="1.2"/><path d="M36 18C46.5 18 55 26.5 55 37C55 47.5 46.5 56 36 56C25.5 56 17 47.5 17 37C17 26.5 25.5 18 36 18Z" fill="none" stroke="url(#portalRing)" stroke-width="7" stroke-linecap="round"/><path d="M24 37C24 30.4 29.4 25 36 25C42.6 25 48 30.4 48 37" fill="none" stroke="#FFFFFF" stroke-width="4.5" stroke-linecap="round"/><circle cx="36" cy="37" r="5.5" fill="#020617" stroke="rgba(255,255,255,.86)" stroke-width="2.2"/><path d="M31 50L36 56L41 50" fill="none" stroke="#C4B5FD" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>重置密码</h1><p class="err">{esc(err)}</p><input name="email" placeholder="注册邮箱"><button class="btn btn2" formaction="/forgot/send" formmethod="post">发送重置验证码</button><input name="code" placeholder="邮箱验证码"><input name="password" type="password" placeholder="新密码，至少 8 位"><button class="btn" formaction="/forgot/reset" formmethod="post">确认重置</button><p class="muted"><a href="/login">返回登录</a></p></form></div></body></html>')
    ...
    def key_view_page(self,u):
        ...
        try:
            ...
            key=res.get('key','')
            body=f'<div class="card"><h2>查看 API Key #{esc(kid)}</h2><p class="muted">请妥善保存，不要在聊天、邮件或工单中发送完整 Key。</p><pre>{esc(key)}</pre><p class="muted">列表显示：{esc(mask_key(key))}</p><div class="actions"><a class="btn" href="/keys">返回</a></div></div>'
        except Exception as e:
...
<div class="card"><h2>错误与限制</h2><p>常见限制包括 Key 禁用、余额不足、模型未授权、套餐额度不足、请求过快、异常路径或违反服务条款。请根据返回错误码、控制台用量和通知记录排查。</p><p class="muted">请勿在客户端、截图、日志或工单中暴露完整 API Key。公开接入仅展示 OpenAI-compatible 协议，不披露内部服务实现和内部链路。</p><div class="actions">{COMPLIANCE_LINKS}<a class="btn btn2" href="/plans">查看套餐</a></div></div>"""
    ...
    def admin_update(self,u):
        ...
        try:
            ...
            log_tail=str(e)
        if st.get('available'):
            status=f'<div class="card" style="border-color:#fde68a;background:#fffbeb"><h2>发现新版本</h2><p>当前版本：<b>{esc(APP_VERSION)}</b></p><p>本地提交：{esc(st.get("local"))}</p><p>Git 最新：{esc(st.get("remote"))}</p><form method="post" action="/admin-update/apply" class="actions">{sensitive_fields(u,"更新备注")}<button class="btn">拉取 Git 并更新</button></form></div>'
        elif st.get('ok'):
        ...
        body=status+f'<div class="card"><h2>更新说明</h2><p>此入口仅管理员可见；点击更新后按 Sub2API 类似模式执行：Git 拉取最新代码 → 部署脚本覆盖正式文件 → 重启门户服务。</p><p class="muted">仓库目录：{esc(REPO_DIR)}</p></div><div class="card"><h2>最近更新日志</h2><pre>{esc(log_tail or "暂无日志")}</pre></div>'
