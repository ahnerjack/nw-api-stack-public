...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL=os.environ.get('NW_API_BASE_URL','https://api.example.com/v1'); APP_VERSION='V0.0.002'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
DB='/opt/xapi-portal/xapi_portal.db'; DATA_BASE=os.environ.get('XAPI_DATA_BASE','http://127.0.0.1:18066/xapi-data')
ADMIN_EMAIL=os.environ.get('XAPI_ADMIN_EMAIL','admin@xapi.local'); ADMIN_PASS=os.environ.get('XAPI_ADMIN_PASS','<SET_VIA_ENV>')
...
def shell(title, body, user, active='dashboard'):
    ...
    return f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="layout"><aside class="side"><a class="logo" href="/dashboard"><span class="mark">N</span><span>{BRAND}<br>{version_update_widget(user)}</span></a><div class="nav">{nav}</div></aside><main class="main"><div class="top">{heading}<a class="btn btn2" href="/">返回首页</a></div>{admin_update_banner(user)}{body}</main></div></body></html>'
...
def public_page(title, body):
    ...
    return f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{css}</head><body><main class="main" style="margin:auto"><div class="top"><a class="logo" href="/home" style="color:#101828"><span class="mark">N</span><span>{BRAND}<br><small class="muted">AI API Gateway · {APP_VERSION}</small></span></a><div class="actions">{nav}</div></div><h1>{esc(title)}</h1>{body}<footer class="muted" style="margin-top:34px">© 2026 NW-API · <a href="/privacy">隐私政策</a> · <a href="/terms">服务条款</a> · <a href="/docs">接入文档</a></footer></main></body></html>'
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.redirect('/dashboard')
    def login(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark">N</span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>登录控制台</h1><p class="err">{esc(err)}</p><input name="email" placeholder="邮箱" autofocus><input name="password" type="password" placeholder="密码"><button class="btn">登录</button><p class="muted">没有账号？<a href="/register">注册</a> · <a href="/forgot">忘记密码</a></p></form></div></body></html>')
    def register(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark">N</span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>注册账号</h1><p class="err">{esc(err)}</p><input name="email" placeholder="邮箱"><button class="btn btn2" formaction="/send-code" formmethod="post">发送邮箱验证码</button><input name="code" placeholder="邮箱验证码"><input name="password" type="password" placeholder="登录密码，至少 8 位"><input name="username" placeholder="昵称，可选"><button class="btn">注册普通会员账号</button><p class="muted">已有账号？<a href="/login">登录</a></p></form></div></body></html>')
    def forgot(self,err=''):
        self.sendh(f'<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{BRAND}</title>{CSS}</head><body><div class="login"><form class="box" method="post"><div class="logo" style="color:#101828"><span class="mark">N</span><span>{BRAND}<br><small class="muted">{APP_VERSION}</small></span></div><h1>重置密码</h1><p class="err">{esc(err)}</p><input name="email" placeholder="注册邮箱"><button class="btn btn2" formaction="/forgot/send" formmethod="post">发送重置验证码</button><input name="code" placeholder="邮箱验证码"><input name="password" type="password" placeholder="新密码，至少 8 位"><button class="btn" formaction="/forgot/reset" formmethod="post">确认重置</button><p class="muted"><a href="/login">返回登录</a></p></form></div></body></html>')
