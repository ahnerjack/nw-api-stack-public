...
HOST='127.0.0.1'; PORT=18180; BRAND='NW-API'; BASE_URL='https://api.example.com/v1'; APP_VERSION='V.0.0.006'; REPO_DIR=os.environ.get('NW_API_REPO_DIR','/opt/nw-api-stack'); UPDATE_LOG=os.environ.get('NW_API_UPDATE_LOG','/var/log/nw-api-update.log')
...
def inline_update_log(st):
    try:
        ...
        out=run(f'cd {shlex.quote(REPO_DIR)} && git log --oneline --no-decorate {shlex.quote(local)}..{shlex.quote(remote)} --', 8)
        lines=[esc(x) for x in out.splitlines()[:5] if x.strip()]
        return '<div class="release-log"><b>更新日志</b><ul>'+''.join(f'<li>{x}</li>' for x in lines)+'</ul></div>' if lines else '<div class="release-log muted">发现新版本，暂无提交摘要。</div>'
    except Exception as e:
...
def release_version_menu(label, user=None, can_update=False, st=None):
    ...
    if user and user.get('role')=='admin' and can_update:
        update_html=f'<form method="post" action="/admin-update/apply" class="release-update-form" onsubmit="return nwInlineUpdate(this.querySelector(\'button\'))">{csrf_field(user)}<button type="submit">立即更新</button><div class="inline-update-result"></div>{inline_update_log(st or {})}</form>'
    return f'<details class="release-menu"><summary><span class="version-pill">{label}</span></summary><div><a href="https://github.com/ahnerjack/nw-api-stack-public/releases/tag/V.0.0.006" target="_blank" rel="noopener">查看发布</a>{update_html}</div></details>'
