...
已在 `10.0.1.66` 部署阶段 0 真实预览环境：

- 入口：`http://10.0.1.66:9088/`
- 后台：`http://10.0.1.66:9088/nv-api/`
- 预览数据 mock：`127.0.0.1:19081`
- 预览 Portal：`127.0.0.1:19080`
- 预览 wrapper：`127.0.0.1:19082`

测试结果：

- 本地 wrapper 回归：5 轮，50/50 通过。
- 真实预览环境：10 轮，50/50 通过。
- 并发无效 key：30/30 正确返回 401。
- 性能基线已记录在 `preview-9088-report.md`。

预览环境已禁用在线更新，并将 `XAPI_DATA_BASE` 指向独立 mock，避免污染生产 Sub2API 数据。
